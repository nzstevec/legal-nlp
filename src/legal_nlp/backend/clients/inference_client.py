...
class InferenceClient:
    def __init__(self):
        self.chat_uri = Config.RUNPOD_BASE_URI + "api/v1/chat"
        self.stream_uri = Config.RUNPOD_BASE_URI + "api/v2/stream"


    def queue_async_job(self, messages, stream=False,  generation_args={}, prompt=None):
        if stream:
            self.get_gpt_stream(messages, stream=stream,  generation_args=generation_args, prompt=prompt)
        else:
            self.get_gpt_response(messages, stream=stream,  generation_args=generation_args, prompt=prompt)
